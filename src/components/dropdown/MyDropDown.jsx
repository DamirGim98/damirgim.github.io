...
const MyDropDown = ({theme, filter, setFilter, items, searchName}) => {
    ...
    return (<div className={cx("dropdown", {"dropdown_dark": theme === "dark"}, {"dropdown_down": isActive})}>
        <div
            className={cx("dropdown_btn")}
        >
            {selected}
            <div>
                {selected !== searchName ? <CloseIcon
                    onClick={(event) => {
                ...
                /> : null}
                {isActive ? <ArrowDropDownIcon onClick={() => setIsActive(!isActive)}
